...
CHData::CHData(const size_t nSitios, const double h, CHData::DConfig config)
    : VecDoub (nSitios), config(config), h(h), invh(1/h)
{
    switch(config)
    {
    case acc1:
...
double CHData::val(int Cn) const
{
    ...
    return (*this)[size_t(Cn)];
}

double CHData::getD1Val(int Cn) const
{
    ...
    #endif
    {
        for(unsigned int i = 0; i<nc; i++)
            num += dc[i]*val(Cn-int(lim+i));
    }

    return invden*num;
}

double CHData::getD2Val(int Cn) const {
...
double CHData::getD3Val(int Cn) const
{
...
double CHData::getD4Val(int Cn) const
{
